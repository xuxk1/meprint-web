...
export default {
  ...
  methods: {
    ...
    //避免缓存
    getNavOpenState() {
      let res = window.localStorage.getItem('navigator-hidden');
      return res;
    },
    ...
    getZoomRadio(value) {
      let zoomStack = this.minder.getOption && this.minder.getOption('zoom');
      if (!zoomStack) {
        return;
      }
      let minValue = zoomStack[0];
      let maxValue = zoomStack[zoomStack.length - 1];
      let valueRange = maxValue - minValue;
      return 1 - (value - minValue) / valueRange;
    },

    getHeight(value) {
      let totalHeight = $('.zoom-pan').height();
